...
namespace SPICA.WinForms.Formats
{
    class FileIO
    {
        ...
        public static void Save(H3D Scene, int pokemon, SceneState State)
        {
            int MdlIndex = State.ModelIndex;
            int[] SelectedAnimations = State.SklAnimIndices;

            new DAE(Scene, MdlIndex, SelectedAnimations).Save(pokemon + ".dae", pokemon);
            //if (Scene == null)
            //{
            //    MessageBox.Show(
        ...
        public static void Export(H3D Scene, int pokemon ,int Index = -1)
        {
            if (Index != -1)
            {
                //Export one
            ...
            else
            {
                ...
                {
                    string FileName = "pokemon/" + pokemon.ToString() + "/" + Scene.Textures[i].Name + ".png";
