...
namespace SPICA.Formats.CtrH3D.Model
{
    ...
    public class H3DBone : INamed
    {
        ...
        public void CalculateTransform(H3DDict<H3DBone> Skeleton)
        {
            ...
            H3DBone Bone = this;
            /*Matrix4x4 OwnTransform = Bone.Transform;

            Matrix4x4 prevTransform = Transform;
            System.Collections.Generic.List<int> BoneChain = new System.Collections.Generic.List<int>();
            while (true)
            {
                if (Bone.ParentIndex == -1) break;
                BoneChain.Add(Bone.ParentIndex);
                Bone = Skeleton[Bone.ParentIndex];
            }

            for (int b = BoneChain.Count-1; b >= 0; b--)
            {
                Bone = Skeleton[BoneChain[b]];
                prevTransform = Bone.Transform * prevTransform;
            }
            Transform = prevTransform * OwnTransform;*/
            //Ppohlus: This seems to be the wrong way to calculate the global transform of the bones. We need to start at the final parent bone and walk up the chain, rather than the inverse which this code does.
            // src: https://docs.godotengine.org/en/3.0/tutorials/math/matrices_and_transforms.html#matrix-tips
            while (true)
            {
                Transform *= Bone.Transform;
